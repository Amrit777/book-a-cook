...
class BaseActiveRecord extends \yii\db\ActiveRecord
{
     ...
     *
     */
    public function getFile($model, $modelId = null, $type = null, $sizeOne = false)
    {
        if ($modelId !== null)
            $model_id = $modelId;
        else
     ...
     */
    public function fileExists($model)
    {
        $data = [];
        $data['status'] = 'NOK';
        $file = $this->getFile($model, null, null, true);
        if (! empty($file)) {
            if (file_exists(UPLOAD_PATH . '/' . $file->file_name)) {
                $data['status'] = 'OK';
